...
class Equation {
    ...
    for (let i = 0; i < operation_priority.length; ++i) {
      for (let j = 0; j < operation_priority[i].length; ++j) {
        ...
        if (!(left_token instanceof TreeNode))
          if (left_token.GetType() == EQUATION_TOKENS.NUMBER)
            //validate left is numeric value
            left_token = new TreeNode(
              EQUATION_TOKENS.NUMBER,
              left_token.GetValue()
            );
            ...
            throw new Error(
              "Syntax Error. Operation must be between 2 numbers"
            );

        if (!(right_token instanceof TreeNode))
          if (right_token.GetType() == EQUATION_TOKENS.NUMBER)
            // validate right is numeric value
            right_token = new TreeNode(
              EQUATION_TOKENS.NUMBER,
              right_token.GetValue()
            );
        ...
        let operation_node = new TreeNode(
          operation_token.GetType(),
          operation_token.GetValue()
        );
  ...
  Tokenize(_equationString) {
    ...
    for (let i = 0; i < PRIORITY_MAX + 1; ++i) operation_priority[i] = [];
    ...
    for (const c of _equationString) {
      ...
      if (parenthesis_counter > 0) {
        if (c == "(") ++parenthesis_counter;
        else if (c == ")") --parenthesis_counter;

        if (parenthesis_counter > 0) parenthesis_content += c;
        else {
          const new_token = new Token(
            current_token_elem,
            EQUATION_TOKENS.NESTED,
            parenthesis_content
          );
  ...
  GetTokenType(_c) {
    ...
    if (_c.charCodeAt(0) > 47 && _c.charCodeAt(0) < 58) return CHAR_TYPE.NUMBER;

    // numbers can be decimals, so "." is still part of a number
    if (_c == ".") return CHAR_TYPE.NUMBER;
  ...
  Calculate() {
    ...
    switch (this.m_type) {
      ...
      case EQUATION_TOKENS.EXPONENT:
        return Math.pow(
          this.m_leftChild.Calculate(),
          this.m_rightChild.Calculate()
        );
    }
  }
